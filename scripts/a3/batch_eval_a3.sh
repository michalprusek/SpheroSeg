#!/bin/bash
# Batch eval all A3 trained models on SpheroMix test (HQ + DTS stratified).
# Runs sequentially on A100 (CUDA_VISIBLE_DEVICES=0), 5-10 min per model.
set -e
PY=/home/prusek/.conda/envs/spheroid_ft/bin/python
SCRIPT=/disk1/prusek/SpheroSeg/code/scripts/a3/evaluate_a3.py
OUT=/disk1/prusek/SpheroSeg/eval_a3/results
LOG=/disk1/prusek/SpheroSeg/logs/batch_eval.log
CKPT=/disk1/prusek/SpheroSeg/checkpoints

# (model_key, weights_path, output_name)
JOBS=(
  ...
  # CBAM ft a5000 added later when training finishes
)

mkdir -p "$OUT"
echo "===== batch eval start $(date -Iseconds) =====" > "$LOG"
...
for job in "${JOBS[@]}"; do
  ...
  echo "----- $(date -Iseconds) eval $model from $weights -----" >> "$LOG"
  CUDA_VISIBLE_DEVICES=0 "$PY" "$SCRIPT" \
    --weights "$weights" --model "$model" --output "$OUT/$out" \
    --batch_size 4 --num_workers 4 >> "$LOG" 2>&1
  echo "----- done rc=$? -----" >> "$LOG"
done

echo "===== batch eval finished $(date -Iseconds) =====" >> "$LOG"

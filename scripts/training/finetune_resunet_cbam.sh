#!/bin/bash
# Finetune script for ResUNet with CBAM on spheroid dataset
...
PRETRAINED_PATH=""  # Will be set via command line argument or auto-detected
...
# Auto-detect latest pretrained model if not specified
if [ -z "$PRETRAINED_PATH" ]; then
    echo "Searching for latest pretrained model..."
    LATEST_PRETRAIN=$(find ./outputs -name "*resunet_cbam_pretrain*" -type d | sort | tail -1)
    if [ -n "$LATEST_PRETRAIN" ] && [ -f "$LATEST_PRETRAIN/best_model.pth" ]; then
        PRETRAINED_PATH="$LATEST_PRETRAIN/best_model.pth"
        echo "Found pretrained model: $PRETRAINED_PATH"
    else
        echo "No pretrained model found! Please specify --pretrained_path or run pretrain_resunet_cbam.sh first"
        exit 1
    fi
fi
...
BATCH_SIZE=6           # Smaller batch size due to CBAM
IMG_SIZE=1024
...
OPTIMIZER="adamw"
SCHEDULER="reduce"     # ReduceLROnPlateau for better stability
...
echo "======================================================================"
echo "ResUNet CBAM Finetuning Configuration"
echo "======================================================================"
echo "Model: $MODEL"
...
try:
    ...
    # Create new model
    model = ResUNetCBAM(in_channels=3, out_channels=1, dropout_rate=0.15)

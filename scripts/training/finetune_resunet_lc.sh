...
if [ -z "$PRETRAINED_PATH" ]; then
    ...
    LATEST_PRETRAIN=$(find ./outputs -name "*resunet_small_pretrain*" -type d | sort | tail -1)
    if [ -n "$LATEST_PRETRAIN" ] && [ -f "$LATEST_PRETRAIN/best_model.pth" ]; then
        ...
        echo "No pretrained model found! Please specify --pretrained_path or run pretrain_resunet_small.sh first"
        exit 1
    fi
fi
...
LEARNING_RATE=1e-5     # Much lower LR for stability (reduced from 5e-5)

#!/bin/bash
# Finetune script for basic UNet on spheroid dataset
...
if [ -z "$PRETRAINED_PATH" ]; then
    ...
    LATEST_PRETRAIN=$(find ./outputs -name "*unet_pretrain*" -type d | sort | tail -1)
    if [ -n "$LATEST_PRETRAIN" ] && [ -f "$LATEST_PRETRAIN/best_model.pth" ]; then
        ...
        echo "No pretrained model found! Please specify --pretrained_path or run pretrain_unet.sh first"
        exit 1
    fi
fi
...
BATCH_SIZE=10          # Same as pretrain for UNet
IMG_SIZE=1024
...
echo "======================================================================"
echo "UNet Finetuning Configuration"
echo "======================================================================"
echo "Model: $MODEL"
